...
//Line legend绘制
function drawLegendLine(series, opts, config, context) {
    if (!opts.legend) {
        return;
    }
    // each legend shape width 15px
    ...
    var _calLegendData = calLegendData(series, opts, config),
        legendList = _calLegendData.legendList,
        legendHeight = _calLegendData.legendHeight;
    var legendList_temp = legendList[0];
    for (var i = 0; i < legendList_temp.length; i++) {
        if (legendList_temp[i].name == undefined) {
            legendList_temp.splice(i, 1);
        }
        for (var y = 0; y < legendList_temp.length; y++) {
            if (legendList_temp[y].name == legendList_temp[i].name) {
                legendList_temp.splice(y, 1);
            }
        }
    }

    legendList[0] = legendList_temp;
        ...
        case 'line':
            this.animationInstance = new Animation({
                ...
                onProcess: function onProcess(process) {
                    ...
                    drawLegendLine(opts.series, opts, config, context);
